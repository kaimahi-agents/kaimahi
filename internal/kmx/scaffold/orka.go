...
// DefaultOrkaInstructions is the system message GenerateOrka supplies when a
// caller provides none. It is exported so that a caller which must state the
// same instructions before generation — encoding flag-based shorthand into a
// portable document, whose closed schema requires them — uses this exact
// text instead of a second copy that could drift from the generated bytes.
...
// DefaultOrkaSecretKey is the Secret key GenerateOrka supplies when a caller
// names none. Like DefaultOrkaInstructions it is exported so that a caller
// which must state the same default before generation — encoding flag-based
// shorthand into a portable document, whose digest identifies what was
// authored — uses this exact value rather than a second copy that could
// drift from the generated bytes.
