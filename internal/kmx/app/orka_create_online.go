...
// createOrkaOnline is the staged online create every existing caller (lift
// included) still uses. The runtime adapter's Deploy needs the created
// Agent's identity, so the body lives in createOrkaStaged and this remains
// its unchanged, identity-free wrapper.
func (a *App) createOrkaOnline(ctx context.Context, opt CreateOptions, bundle *scaffold.OrkaBundle) error {
	_, err := a.createOrkaStaged(ctx, opt, bundle)
	return err
}

// createOrkaStaged runs the staged create and returns the created Agent's
// identity. A --dry-run creates nothing and therefore returns none.
func (a *App) createOrkaStaged(ctx context.Context, opt CreateOptions, bundle *scaffold.OrkaBundle) (agent orkaIdentity, err error) {
	...
	for _, kind := range []string{"Agent", "Provider", "Task"} {
		...
		if err != nil {
			return agent, fmt.Errorf("cannot read installed %s CRD (no offline fallback): %w", name, err)
		}
		crds[kind] = raw
	}
	...
	if a.liftReuse && bundle.Task != nil {
		return agent, fmt.Errorf("lift cannot reuse or resubmit Tasks")
	}
	...
	if err != nil {
		return agent, fmt.Errorf("cannot read Provider Secret key presence: %w", err)
	}
	switch string(marker) {
	case "":
		return agent, fmt.Errorf("Provider Secret %s/%s is missing; provision it separately, never create the skeleton", opt.Namespace, opt.Secret)
	case "secret\n":
		return agent, fmt.Errorf("Provider Secret exists but its referenced key is missing; provision that key separately")
	case "secret\npresent":
	default:
		return agent, fmt.Errorf("Provider Secret key check returned an invalid presence marker")
	}
	report("done", nil)
	...
	for _, doc := range bundle.Documents()[1:] {
		...
		if _, err := a.orkaCapture(ctx, body, "-n", opt.Namespace, "create", "--dry-run=server", "--validate=strict", "-f", "-", "-o", "json"); err != nil {
			return agent, fmt.Errorf("%s strict server create preflight failed: %w", doc["kind"], err)
		}
	}
	if opt.DryRun {
	...
	for _, doc := range []map[string]any{bundle.Provider, bundle.Agent} {
		...
		// The returned identity is the Agent this create actually proved
		// Ready, so a caller can never be handed a reference to an object
		// that was only submitted.
		if id.Kind == "Agent" {
			agent = id
		}
		report("done", nil)
