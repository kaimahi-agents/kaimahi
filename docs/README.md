...
| AKS | **Demonstrated, not maintained.** Five verified runs across 2026-09-01, 09-02 and 09-06 (the plane, NetworkPolicy enforcement, the Slack loop through a public edge, and `kmx lift` on a cluster it created and one it did not), each cluster deleted the same day. CI stays on kind and keyless ([aks.md](aks.md)) |
