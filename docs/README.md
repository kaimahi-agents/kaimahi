...
[kmx.md](kmx.md) is the command reference. Make is used only for contributor
builds and repository demo orchestration where no installed command exists.

## By what you want to do

| I want to… | Read | The commands |
|---|---|---|
| Get an agent running and talk to it | [getting-started.md](getting-started.md) | `kmx up`, `kmx agent chat`, `kmx status`, `kmx down` |
...
| Create an Orka Agent and Provider as reviewable YAML, optionally run a Task | [kmx.md](kmx.md#kmx-agent-create) | `kmx agent create <name> --namespace <ns> --provider-type openai --model <id> --secret <name>` |
| Use a hosted model (Copilot, Anthropic, OpenAI, Azure AI Foundry, OpenRouter, anything OpenAI-compatible) | [models.md](models.md) | `kmx models credential copilot`, `kmx use …` |
| Give the agent a tool | [tools.md](tools.md) | `kmx agent chat hello-tools …` |
| Put a budget on the agent's LLM spend, see the ledger, keep real API keys away from the agent | [spend.md](spend.md) | `kmx plane`, `kmx govern`, `kmx budget`, `kmx ledger` |
| Control which tools the agent can call, and audit every call | [tool-governance.md](tool-governance.md) | `kmx tools govern`, `kmx tools allow`, `kmx audit tool` |
...
| Have a human approve a denied action with a bounded, expiring grant | [approvals.md](approvals.md) | `kmx approvals`, `kmx approve`, `kmx grants` |
| Let the repository demo agent post to Slack, one approved message at a time | [slack.md](slack.md) | checkout-only orchestration: `make slack-secret`, `make slack-mcp`, `make govern-slack`, `make slack-post` |
...
| Let the repository demo agent read issues and pull requests through GitHub's hosted MCP server, with the token in plane custody | [hosted-upstreams.md](hosted-upstreams.md) | `kmx credential capture github owner/name`; checkout-only Make orchestration deploys the demo agent |
...
| Cut a real release: an agent drafts the notes from what merged, proposes the branch and the builds, a human approves the branch cut and the publish by version and repository, and builds run under a standing constraint | [release-agent.md](release-agent.md) | `kmx workflow govern release`, `kmx workflow run release` |
| Say what a workflow reaches, what may happen without you, and in what order — in one file, and run it | [workflows.md](workflows.md) | `kmx workflow list`, `kmx workflow show`, `kmx workflow govern`, `kmx workflow refresh`, `kmx workflow run` |
| Show the whole thing on money: an invoice that does not match, an answer a person has to approve by amount and payee, and a later invoice that tries to redirect the payment — or [tell it without running it](ap-demo.md#telling-it-without-running-it) | [ap-demo.md](ap-demo.md) | checkout-only repository demo: `make erp`, `make govern-ap`, `make ap-demo`, `make ap-injection` |
| Install Orka itself, from a cluster that has nothing on it, with no API key anywhere | [orka.md](orka.md) | `kmx orka install`, then `kmx orka status` |
| Put an application I did not write onto Orka, with its model traffic authenticated, Provider-scoped and recorded, without changing the application | [migrate.md](migrate.md) | `kmx migrate <deployment> --namespace … --model …`, then the one `kubectl patch` it prints |
...
| Know who an agent acted for, and stop a token living forever | [identity.md](identity.md) | `kmx credentials`, `kmx credential renew`, `kmx ledger`, `kmx audit inbound` |
| Run the plane for real: replicas, probes, metrics, back up and restore the ledger | [operations.md](operations.md) | `kmx backup`, `kmx restore`, `kmx metrics` |
| Run all of this on a real cluster (AKS) instead of kind | [aks.md](aks.md) | `kmx lift`, `kmx lift down` |
...
| Tool calls through `kaimahi-tools` (after `kmx tools govern`) | **Governed**: authentication, a committed upstream table, tools-only protocol scope, a per-credential allowlist, every call audited ([tool-governance.md](tool-governance.md)) |
| Tool calls through `kagent-tool-server` directly | **Ungoverned, by choice.** Run `kmx tools govern` to govern it |
